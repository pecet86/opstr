...
class Dijkstra {
		...
		if (!empty($this->graph[$closest]) && !in_array($closest, $exclude)) {
			foreach ($this->graph[$closest] as $neighbor => $cost) {
				...
				if (isset($this->distance[$neighbor])) {
					if ($this->distance[$closest] + $cost[0] < $this->distance[$neighbor]) {
						// A shorter path was found
						$this->distance[$neighbor] = $this->distance[$closest] + $cost[0];
						$this->previous[$neighbor] = array(array($closest, $cost[1], $cost[0]));
						$this->queue[$neighbor]    = $this->distance[$neighbor];
					} elseif ($this->distance[$closest] + $cost[0] === $this->distance[$neighbor]) {
						// An equally short path was found
						$this->previous[$neighbor][] = array($closest, $cost[1], $cost[0]);
						$this->queue[$neighbor]      = $this->distance[$neighbor];
					}
				}
			}
	...
	//route
	protected function extractPaths2($paths) {	
		$results = array();
	...
	//distance
	protected function extractPaths3($paths) {	
		$results = array();
		foreach ($paths as $path) {
			$result = array();
			foreach ($path as $node) {
	...
	public function shortestPaths($source, $target, array $exclude = array()) {
		...
		//var_dump(json_encode($this->previous));
		
		if ($source === $target) {
			// A null path
			return array(array(array($source)));
		} elseif (empty($this->previous[$target])) {
			// No path between $source and $target
			return array();
		} else {
			// One or more paths were found between $source and $target
			$path = $this->extractPaths($target);
			return array($path, $this->extractPaths2($path), $this->extractPaths3($path));
		}
	}
}

// No shortest path.
//$path = $dijkstra->shortestPaths('A', 'G'); 
// array()

// Exactly one shortest path.
//$path = $dijkstra->shortestPaths('A', 'E'); 
// array(array(array('A', 'B', 'D', 'E')), array(array('AB', 'BD', 'DE')))

// Multiple solutions with the same shortest path.
//$path = $dijkstra->shortestPaths('E', 'F'); 
// array(array(array('E', 'D', 'B', 'F'), array('E', 'C', 'F')), array(array('AB', 'BD', 'DE'), array('EC', 'CF')))
